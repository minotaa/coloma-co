...
const RETREAT_DISTANCE := 120.0  # Try to maintain this distance from the player
const APPROACH_DISTANCE := 180
const APPROACH_SPEED = 120.0
const RETREAT_SPEED = 70.0
...
func shoot_at_player(target_pos: Vector2) -> void:
	...
	var bullet1 = preload("res://scenes/bullet.tscn").instantiate()
	bullet1.global_position = global_position
	bullet1.direction = base_dir
	get_tree().current_scene.add_child(bullet1)

	var bullet2 = preload("res://scenes/bullet.tscn").instantiate()
	bullet2.global_position = global_position
	bullet2.direction = base_dir.rotated(deg_to_rad(45))
	get_tree().current_scene.add_child(bullet2)

	var bullet3 = preload("res://scenes/bullet.tscn").instantiate()
	bullet3.global_position = global_position
	bullet3.direction = base_dir.rotated(deg_to_rad(-45))

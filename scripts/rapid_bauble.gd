...
const RETREAT_DISTANCE := 80.0  # Try to maintain this distance from the player
const APPROACH_DISTANCE := 100
const APPROACH_SPEED = 30.0
const RETREAT_SPEED = 10.0
...
func _ready() -> void:
	...
	entity.health = 320.0
	entity.max_health = 320.0
	entity.defense = 0.0
	entity.name = "Rapid Bauble"
	entity.id = 9
...
func take_damage(amount: float, from_position: Vector2, name: String) -> void:
	...
	if entity.health <= 0 and alive:
		...
		get_parent().add_kill(name, "rapid_bauble")
